...
from scripts.truevision_studio_server import (
...
class TrueVisionStudioServerTests(unittest.TestCase):
    ...
    def test_build_generation_template_matches_audio_duration_when_present(self):
        request = {
            ...
            "state_plan": {"scene": {"name": "wake river"}},
        }
    ...
    def test_resolve_assistant_actions_turns_request_into_work(self):
        request = {}

        actions = resolve_assistant_actions("compile state then prepare the record command", request)
    ...
    def test_resolve_assistant_actions_routes_plain_text_to_save_request(self):
        request = {}

        actions = resolve_assistant_actions("what can you help me do in this studio?", request)

        self.assertEqual(actions, ["save_request"])

    def test_handle_assistant_message_saves_plain_text_request(self):
        request = {
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(result["ok"])
            self.assertEqual(result["actions"], ["save_request"])
